...
def convert_to_json(Inverter_data):
	...
	Inverter_datavalue = [
	float(Inverter_data) if '.' in Inverter_data else (int(Inverter_data) if ',' in Inverter_data else int(Inverter_data)) for Inverter_data in re.findall(r'-?\d+\.?\d*',Inverter_data) ]	
	...
	return json.dumps(Inverter_data)

Json_Inverter = convert_to_json(Inverter_data)
...
p = (
        ...
        .tag("Device_ID", Json_Inverter.Device_ID_1)
        .field("Power", Power)
        .field("DC_Input_Power", DC_Input_Power)
